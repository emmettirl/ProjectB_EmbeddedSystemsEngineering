...
struct Triangle Test = {200,200,250,250,280,200};
...
//     uprintf("got vidhandler %x counter %d\n",*(volatile unsigned int *)(0x1012001C),counter);
     ...
     switch(key){
      ...
      for (int i=0;i<pacman_size;i++){
         ...
         if (check){
         /* TODO */
            /* remove the entry in the appropriate position in the pacman_tiles array. Also need to set the isalive to 2 
            in the pacmans array. */
